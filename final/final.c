...
int main()
{
    ...
    for(i=0;i<epoch;i++){
        ...
        for(j=0;j<train_count /batch;j++){
            ...
            //一つ一つの勾配を計算する
            for(k=0;k<batch;k++){
                backward3(A,b,train_x+ 784*index[k] ,train_y[index[k]],y,dEdA,dEdb);
                add(784*10,dEdA,dEdA_av);
                add(10,dEdb,dEdb_av);
            }

            //ミニバッチで割って平均を求める
            scale(784*10,1/batch,dEdA_av);
            scale(10,1/batch,dEdb_av);

            //係数A,bを更新
            scale(784*10,-1*h,dEdA_av);
            add(784*10,dEdA_av,dEdA);

            scale(10,-1*h,dEdb_av);
            add(10,dEdb_av,dEdb);

        }
        printf("Complete Epoch %d\n",i+1);
        //テストデータでの推論
        int sum = 0;
        for(m=0 ; i<test_count ; m++) {
            if(inference3(A, b, test_x + m*width*height) == test_y[m]) {
                sum++;
            }
        }
